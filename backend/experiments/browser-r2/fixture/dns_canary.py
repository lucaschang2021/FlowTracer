...
EVENTS_PATH = Path(os.environ.get("R2_DNS_EVENTS", "/evidence/dns-events.jsonl"))
_event_lock = Lock()
...
def _question(packet: bytes) -> tuple[str, str]:
    ...
    while offset < len(packet):
        ...
        if length == 0:
            ...
            type_name = {1: "A", 28: "AAAA"}.get(query_type, f"TYPE{query_type}")
            return ".".join(labels).lower(), type_name
...
def _nxdomain(packet: bytes) -> bytes:
    ...
    question_count = packet[4:6]
    # QR=1, AA=1, RD copied as received, RA=0, RCODE=NXDOMAIN. The canary is
    # authoritative for the synthetic test namespace and has no upstream path.
    return transaction_id + b"\x85\x03" + question_count + b"\x00\x00\x00\x00\x00\x00" + packet[12:]
