...
code. `../run_r2c_a3.py` builds the merged R1E Dockerfile once with
`--no-cache --pull=false --platform=linux/amd64`, proves the candidate's full
Runtime Identity v2 manifest is byte-identical to the accepted R1E manifest,
...
The formal evidence path is `../evidence/r2c-a3/`. The runner refuses to
