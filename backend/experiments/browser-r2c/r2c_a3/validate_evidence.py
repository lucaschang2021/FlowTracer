...
def validate_identity(evidence: Path) -> dict[str, Any]:
    ...
    if gate.get("manifest_byte_identical") is not True:
        raise RuntimeError("candidate manifest is not byte-identical to R1E")
...
def validate_runtime(evidence: Path) -> dict[str, Any]:
    ...
    for role, facts in runtime.items():
        ...
        expected_ports = (
            {"49273/tcp": [{"HostIp": "127.0.0.1", "HostPort": "49273"}]}
            if role == "host_canary"
            else {}
        )
    ...
    if (
        ...
        or host_control.get("port") != 49273
...
def main() -> None:
    evidence = Path(sys.argv[1] if len(sys.argv) > 1 else "evidence/r2c-a3")
