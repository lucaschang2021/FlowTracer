...
def validate_identity(evidence: Path) -> dict[str, Any]:
    ...
    if (
        gate.get("build2_identity") != EXPECTED_IDENTITY
        or gate.get("builds_byte_identical") is not True
        or gate.get("manifest_byte_identical") is not True
    ):
        raise RuntimeError("two candidate manifests are not byte-identical to R1E")
    ...
    if supply.get("account_metadata") != {
        ...
        raise RuntimeError("deterministic account metadata gate mismatch")
    if (
...
def validate_runtime(evidence: Path) -> dict[str, Any]:
    ...
    for role, facts in runtime.items():
        ...
        expected_ports = (
            {"49274/tcp": [{"HostIp": "127.0.0.1", "HostPort": "49274"}]}
            if role == "host_canary"
            else {}
        )
    ...
    if (
        ...
        or host_control.get("port") != 49274
    ...
    if (
        ...
        or browser_security.get("tmpfs") != {"/tmp": "rw,nosuid,noexec,size=256m"}  # noqa: S108 - dedicated tmpfs
    ):
    ...
    if (
        contract.get("executable_path") != "/opt/browser-r1c/chromium-1234/chrome-linux64/chrome"
...
def main() -> None:
    evidence = Path(sys.argv[1] if len(sys.argv) > 1 else "evidence/r2c-a4")
    ...
    if (
        ...
        or result.get("historical_r2c_unchanged") is not True
