...
def run(argv: list[str], *, timeout: int = 120, check: bool = True) -> subprocess.CompletedProcess[str]:
    started = now()
    result = subprocess.run(argv, cwd=REPO, capture_output=True, text=True, timeout=timeout)
    ledger.append(
        {
            "argv": argv,
            "started_at": started,
...
def hardened_python(arguments: list[str], *, user: str = "0:0", timeout: int = 300) -> subprocess.CompletedProcess[str]:
    return run(
        [
...
def verify_copied_inputs() -> None:
    ...
    paths = [Path("Dockerfile"), Path("browser-tree.manifest.json"), Path("debian-packages.lock"), Path("requirements.lock")]
...
def cleanup() -> dict[str, object]:
    ...
    for argv in (["docker", "image", "rm", "-f", IMAGE], ["docker", "buildx", "rm", "-f", BUILDER]):
        ...
        actions.append({"argv": argv, "exit_code": result.returncode, "stderr": result.stderr[-1000:]})
    return {
        "actions": actions,
        "builder_absent": run(["docker", "buildx", "inspect", BUILDER], check=False).returncode != 0,
...
def main() -> int:
    ...
    try:
        ...
        absent_before()
        run(["docker", "buildx", "create", "--name", BUILDER, "--driver", "docker-container", "--driver-opt", f"image={BUILDKIT_IMAGE}", "--use", "--bootstrap"], timeout=300)
        build = run(["docker", "buildx", "build", "--builder", BUILDER, "--platform=linux/amd64", "--no-cache", "--pull=false", "--progress=plain", "--load", "--target", "final", "-f", str(ROOT / "Dockerfile"), "-t", IMAGE, str(ROOT)], timeout=1800)
        (EVIDENCE / "identity-build.txt").write_text(build.stdout + build.stderr, encoding="utf-8", newline="\n")

        identity = hardened_python(["/opt/flowtracer-r1c/scripts/image_identity.py"]).stdout.strip()
        manifest_raw = hardened_python(["/opt/flowtracer-r1c/scripts/browser_tree_manifest.py"]).stdout
        (EVIDENCE / "browser-tree.actual.json").write_text(manifest_raw, encoding="utf-8", newline="\n")
        manifest = json.loads(manifest_raw)

        facts_raw = hardened_python(["-c", "import hashlib,json,pathlib,subprocess; p=pathlib.Path('/opt/browser-r1c/chromium-1234/chrome-linux64/chrome'); print(json.dumps({'path':str(p),'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'version':subprocess.check_output([str(p),'--version'],text=True).strip()}))"]).stdout
        facts = json.loads(facts_raw)

        sbom_raw = hardened_python(["/opt/flowtracer-r1c/scripts/generate_sbom.py"], timeout=600).stdout
        inventory_raw = hardened_python(["/opt/flowtracer-r1c/scripts/generate_sbom.py", "--debian-license-inventory"], timeout=600).stdout
        ...
        checks = {
            "normalized_identity": {"expected": EXPECTED_IDENTITY, "actual": identity, "match": identity == EXPECTED_IDENTITY},
            "browser_path": {"expected": "/opt/browser-r1c/chromium-1234/chrome-linux64/chrome", "actual": facts["path"], "match": facts["path"] == "/opt/browser-r1c/chromium-1234/chrome-linux64/chrome"},
            "browser_sha256": {"expected": EXPECTED_BROWSER_SHA, "actual": facts["sha256"], "match": facts["sha256"] == EXPECTED_BROWSER_SHA},
            "browser_version": {"expected": "Google Chrome for Testing 151.0.7922.34", "actual": facts["version"], "match": facts["version"] == "Google Chrome for Testing 151.0.7922.34"},
            "browser_tree_entries": {"expected": 619, "actual": len(manifest["entries"]), "match": len(manifest["entries"]) == 619},
            "sbom_components": {"expected": 231, "actual": len(sbom["components"]), "match": len(sbom["components"]) == 231},
            "sbom_sha256": {"expected": EXPECTED_SBOM_SHA, "actual": sha256(sbom_path), "match": sha256(sbom_path) == EXPECTED_SBOM_SHA},
            "debian_inventory_components": {"expected": 206, "actual": len(inventory["components"]), "match": len(inventory["components"]) == 206},
            "debian_inventory_sha256": {"expected": EXPECTED_INVENTORY_SHA, "actual": sha256(inventory_path), "match": sha256(inventory_path) == EXPECTED_INVENTORY_SHA},
        }
