...
HARNESS = ROOT / "r2c_a3"
EVIDENCE = ROOT / "evidence" / "r2c-a3"
RUN_ID = "flowtracer-r2c-a3-20260920-final-a"
PROJECT = RUN_ID
...
def ensure_clean_start() -> None:
    if not re.fullmatch(r"flowtracer-r2c-a3-[a-z0-9][a-z0-9-]{7,80}", RUN_ID):
...
"""
    ...
    checks = {
        ...
        "shadow_in_identity": any(
            ...
            for entry in json.loads(identity_raw)["entries"]
        ),
    }
    gate = {
        "identity": identity,
