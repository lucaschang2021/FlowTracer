...
**当前停点：R2C-A2 因跨日 `/etc/shadow` identity 漂移 BLOCKED；仅 R1E 确定性账户修复准入**

Architecture Governance Pass 已完成。R1D Runtime Identity v2 已通过独立复审、Backend CI 与 PR #63 合并；R2C-A2 次日重建仅发现 `/etc/shadow` 的 `sp_lstchg` 跨日漂移，并在网络矩阵前安全停止。当前仅准入 R1E；R2C、R3、R4-R5、正式 WP-3、Frontend、Integration 与 Release 均未准入。

本次同步事实基准：`main@aac317d5ad1bf6f041c5a75d4ce2da2479bb14ba`（R2C 激活 PR #64 merge commit）。
...
| ACQ-1-WP3-R1E | Deterministic account metadata 与跨日 Runtime Identity v2 修复 | Backend / Architecture | 已准入；待执行 | ACQ-1-WP3-R2C-A2 BLOCKED | ADR-033、`48-ACQ1-WP3-R1E-DETERMINISTIC-ACCOUNT.md`；不得排除 `/etc/shadow` 或放宽 identity |
| ACQ-1-WP3-R2C | R1C/R1D/R1E 完整 Chromium 上的 controlled egress 完整回归 | Backend / Architecture | BLOCKED（P1×1）；暂停 | ACQ-1-WP3-R1E Accepted | `46-ACQ1-WP3-R2C-ACTIVATION.md`；R1E 合并后才可再次激活 |
