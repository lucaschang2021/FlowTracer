...
**当前阶段：R1D 已验收合并；R2C controlled egress 完整回归重新准入**

Architecture Governance Pass 已完成。R1D Runtime Identity v2 已通过独立复审、Backend CI 与 PR #63 合并，逐路径 manifest 和 runtime/audit 边界可由合并后的权威来源重建。当前仅重新准入 R2C；R3、R4-R5、正式 WP-3、Frontend、Integration 与 Release 均未准入。

本次同步事实基准：`main@db9c4fad0ca826b472d18d85c69db53848cfcd94`（R1D PR #63 merge commit）。
...
| ACQ-1-WP3-R2C | R1C/R1D 完整 Chromium 上的 controlled egress 完整回归 | Backend / Architecture | 已重新准入；待执行 | ACQ-1-WP3-R1D Accepted | `46-ACQ1-WP3-R2C-ACTIVATION.md`；仅准入网络矩阵与证据，不含 R3 |
