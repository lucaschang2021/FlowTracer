...
- 当前稳定基准：`main@34364d0082e4ecdbe4331d77a21ef6d25c2fca8a`；该基准包含 WP-3 Remediation Evidence 控制 PR #53 与 R1 证据 PR #54。
...
- 当前停点：WP-3 正式实现继续 BLOCKED。仅准入 R2 Production-shaped controlled egress proxy/namespace 证据阶段；R3-R5 必须继续等待各自上游 PASS，见 `docs/43-ACQ1-WP3-R2-ACTIVATION.md`。
...
下一步：在新任务中执行 R2，仅使用本地 fixture/保留地址证明 Browser namespace 只能经受控 proxy 到达声明目标，并关闭 DNS、host gateway、Docker socket、直连与 CONNECT/redirect/rebinding 旁路。R2 PASS 并经独立复审前不得进入 R3。R1→R5 全部通过也只可申请独立总控复核；仍须另提 WP-3 Contract Addendum + 正式 Admission 控制 PR 并合并后，才可进入正式实现。
